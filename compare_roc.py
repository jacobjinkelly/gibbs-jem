"""
Plot ROC curves for different models alongside each other.
"""
...
def plot_roc_curve(dirs, model_names, fn_prefix):
    plt.clf()
    ...
    for exp_dir, model_name in zip(dirs, model_names):
        ...
        plt.plot(fpr, tpr, label=f"{model_name}", color=c)
    ...
    for exp_dir in dirs:
        plt.savefig(f"{exp_dir}/compare_roc_{fn_prefix}.png")
...
def main():
    ...
    for label_dim in range(count_caches_in_dir(args.dirs[0])):
        plot_roc_curve(args.dirs, args.model_names, label_dim)

    plot_roc_curve(args.dirs, args.model_names, "micro")
