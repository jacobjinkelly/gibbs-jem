...
def plt_flow_density(logdensity, ax, npts=100, title="$q(x)$", device="cpu", low=LOW, high=HIGH, exp=True,
                     return_norm=False):
    side = np.linspace(low, high, npts)
    xx, yy = np.meshgrid(side, side)
...
def plt_samples2(samples, lbls, ax, title="$x ~ p(x)$"):
    samples0 = samples[lbls == 0]
    samples1 = samples[lbls == 1]
    samples0 = samples0.cpu().detach().numpy()
    samples1 = samples1.cpu().detach().numpy()
    ax.scatter(samples0[:, 0], samples0[:, 1])
